...
\date{Work items \texttt{mg-c15b} (Phase 1) and \texttt{mg-365e}
  (revision per the \texttt{mg-d300} Phase-2 validation) --- \today\\[2pt]
...
but its reduction now carrying a genuine $\Sym_{n-1}\to\Sym_n$
branching step the $(\Z/2)^n\to\Sym_n$ re-grading creates); the
landing statement $Y_2$ (whose pre-Fork-A mechanism rested on the
...
discharged at the structural level and its remaining open parts ($Y_2$
and the new branching step) are precisely localized; the second owed
...
the open residuals R1 (cofiber plus branching), R2, and R3 (three-part
faithfulness).} This document has been revised following the
\texttt{mg-d300} independent Phase-2 validation; see
Status~\ref{status:mg365e-revision}.
\end{abstract}
...
$Y_1$-A (the vanishing) --- residual R1
 ...
 & \S\ref{ssec:y1a}. The cofiber-LES reduction ports to Fork~A as an
   ...
   (Theorem~\ref{thm:y1-reduction}); the residual is two-part (Open
   residual~\ref{res:y1}) --- R1a, the cofiber, unchanged as a target
   object; R1b, a new $\Sym_{n-1}\to\Sym_n$ branching step.\\
...
Owed debt 1 --- the re-grading
 ...
 & \S\ref{ssec:debt1}. Structural re-grading discharged
   ...
   part is residual R2 \emph{and} the new branching sub-residual
   R1b.\\
...
the residuals R1--R3 (R1 itself two-part, R3 itself three-part). A
...
continues with an independent re-validation of this revised document
and then Phase~3 (Lean), each gated on the previous returning GREEN.
...
H^{n-1}(\Cn;X)$ in the first place. Phase~1 discharges none of the
three.
\end{status}
...
\item Projecting that LES to any $\Sym_{n-1}$-isotype $[\mu]$ (Maschke
  ...
  in each degree, to a relation expressing the $[\mu]$-isotype of the
...
\end{enumerate}
Hence $Y_1$-A reduces to \emph{two} steps: \textup{(a)} control of the
$\Sym_{n-1}$-isotype cohomology of $\cofib\iota_x$, with the
smaller-ground-set terms supplied by the simultaneous induction on
$n$; and \textup{(b)} an induction--restriction (branching) step
recovering the $\Sym_n$-isotype structure of $\widetilde
H^{n-1}(\Cn;X)$ --- in particular its $\sgn_{\Sym_n}$-content, which
the $\sgn_{\Sym_n}$ target of $Y_1$-A (Conjecture~\ref{conj:y1a})
requires --- from the $\Sym_{n-1}$-equivariant data of step~(a). Both
steps are genuinely open (Open residual~\ref{res:y1}: R1a and R1b
respectively).
\end{theorem}
...
which is Maschke for $\Sym_n$, does not apply to this LES. The bridge
(Construction~\ref{constr:bridge}) gives $\iota_x^{*}=0$ on cohomology
in the relevant degrees; substituting the zero map makes the
restriction $j^{*}$ surjective with kernel the image of the connecting
map $\delta$, and $\delta$ injective. This is the
$\Sym_{n-1}$-graded port of the pre-Fork-A cofiber-LES re-derivation
...
class of the $(n-1)$-cube. The pre-Fork-A re-derivation split into
\emph{Walsh} isotypes, for which the $(\Z/2)^n$-action does descend to
the smaller cube; the $\Sym_n$ re-grading has no such descent, which
is the source of the branching step~(b)
(Remark~\ref{rem:y1-branching}). Phase~1 does not claim Fork~A removes
the cofiber delicacy, nor that the reduction ports unchanged: it
claims the reduction ports \emph{to an $\Sym_{n-1}$-equivariant
statement}, and that closing $Y_1$-A then needs both the cofiber and
the branching step.
\end{proof}
...
\begin{openresidual}[The $Y_1$ residual --- R1, in two parts]
\label{res:y1}
$Y_1$-A reduces, by Theorem~\ref{thm:y1-reduction}, to \emph{two}
...
acquires, under Fork~A, the new $\Sym_{n-1}\to\Sym_n$ branching
sub-residual R1b.} Fork~A does not close either part and does not
claim to.
\end{openresidual}
...
two-part (Open residual~\ref{res:y1}): the cofiber R1a --- the
homotopy type of the stuck-family fibres, the same hard object as
before, now correctly housed --- and the new $\Sym_{n-1}\to\Sym_n$
branching sub-residual R1b that the $(\Z/2)^n\to\Sym_n$ re-grading
creates. The pre-Fork-A claim that the reduction ``ports unchanged''
does \emph{not} hold for Fork~A; the honest statement is that it ports
to an $\Sym_{n-1}$-equivariant reduction \emph{plus} a branching step.
\end{status}
...
\item \textup{(R1, vanishing)} $Y_1$-A holds
  ...
  Theorem~\ref{thm:y1-reduction}, itself rests on the two-part $Y_1$
  residual: R1a (the cofiber) and R1b (the $\Sym_{n-1}\to\Sym_n$
  branching step), Open residual~\ref{res:y1}.
\end{enumerate}
...
\item It rests on the named open inputs R1, R2, R3 --- with R1 itself
  two-part (R1a the cofiber, R1b the $\Sym_{n-1}\to\Sym_n$ branching
  step) and R3 itself three-part (R3a existence, R3b degree, R3c
  injectivity of the edge map). Phase~1 makes \emph{no} claim that
  this is a count of ``exactly three'': it is the honest enumeration
  of every input Theorem~\ref{thm:contradiction} rests on. The
  comparison with the pre-Fork-A closing argument is genuine but
  modest: Fork~A removes the dependence on the unconstructed
  $(\Z/2)^n$-action (Status~\ref{status:c-block}) and houses the
  construction on a well-defined object --- a gain in soundness and
  clarity --- but in doing so it trades the $(\Z/2)^n$ input for the
  $\Sym_{n-1}\to\Sym_n$ branching step R1b and surfaces the edge-map
...
\item R1a (the $Y_1$ cofiber residual, as a target object) and R3c
  ...
  state. The inputs Fork~A genuinely changes are three: R2 (the
  ...
  (Remark~\ref{rem:y1-branching}); and R3a/R3b (edge-map existence and
  degree), implicit and unexamined in the pre-Fork-A development's
  undefined object, which Fork~A must now name because its object is
  genuinely defined.
...
named precisely and \emph{in full} --- the residuals R1 (two-part),
...
\emph{surfaces} two inputs that the pre-Fork-A development obtained
...
\item \emph{The $Y_1$ branching step.} The pre-Fork-A cofiber-LES
  ...
  precisely as the new sub-residual R1b (Open residual~\ref{res:y1}).
\end{enumerate}
Neither is a failure of the re-grading; both are the re-grading doing
its job --- exposing that inputs the pre-Fork-A ledger treated as
settled (``$Y_2$ mechanism-sound''; ``$Y_1$ reduction unchanged'') in
fact depended on the now-refuted $(\Z/2)^n$-action and must be
re-established. The discipline is symmetric: dropping $(\Z/2)^n$
re-opens work on \emph{both} sides of the contradiction, and Phase~1
records both.
...
residue is precisely named --- the landing residual R2 and the
branching sub-residual R1b that the re-grading surfaces. The brief
...
residue localized as R2 (the $Y_2$ separation) and R1b (the
$\Sym_{n-1}\to\Sym_n$ branching step the re-grading newly surfaces);
Owed debt~\ref{debt:faithful} (faithfulness) is localized as the
three-part R3. \textbf{Frankl is not proved, and Phase~1 was never
meant to prove it.} The verdict GREEN is for the Phase-1 scope: the
re-foundation is delivered and internally sound. The genuine open
content is named precisely and in full --- R1 (two-part: R1a the
$Y_1$ cofiber, R1b the branching step), R2 (the $Y_2$ landing /
separation), and R3 (three-part: existence, degree, and injectivity of
the \v{C}ech-to-cohomology edge map).
...
\item The $Y_1$ vanishing re-stated as re-graded sphere concentration
  ...
  (Theorem~\ref{thm:y1-reduction}) and the new $\Sym_{n-1}\to\Sym_n$
  branching sub-residual R1b honestly recorded
  (Remark~\ref{rem:y1-branching}).
\item The closing contradiction re-established as a conditional
  ...
  it rests on enumerated in full --- R1 (two-part: R1a cofiber, R1b
  branching), R2, and R3 (three-part: R3a existence, R3b degree, R3c
  injectivity of the edge map) --- and no claim of a clean
  ``exactly three / one fewer than pre-Fork-A'' count
  (Remark~\ref{rem:honest-shape}).
...
\begin{itemize}[leftmargin=1.7em]
\item \textbf{R1 --- $Y_1$-A, Open residual~\ref{res:y1}}, in two
...
\begin{enumerate}[leftmargin=1.9em]
\item \textbf{Proceed to an independent re-validation of this revised
  document.} The first Phase-2 validation (\texttt{mg-d300}) returned
  AMBER and is incorporated: this revision corrects the two
  over-claims it caught (Status~\ref{status:mg365e-revision}). A
  re-validation should default-skeptically confirm that the correction
  is complete and honest --- in particular that
  Theorem~\ref{thm:y1-reduction} now claims only
  $\Sym_{n-1}$-equivariance and that R1b is genuinely recorded
  (Remark~\ref{rem:y1-branching}, Open residual~\ref{res:y1}); that
  Theorem~\ref{thm:contradiction} lists \emph{every} input, including
  the three-part R3 (Owed debt~\ref{debt:faithful-localized}); and
  that the spine --- Theorem~\ref{thm:projection} (chain map and
  functoriality), Theorem~\ref{thm:welldefined} ($D^2=0$),
  Theorem~\ref{thm:maschke} (the $\Sym_n$-equivariance of $D$),
  Proposition~\ref{prop:y2a-mech} --- which \texttt{mg-d300}
  independently confirmed sound, is undisturbed by the revision.
\item \textbf{The $Y_1$ residual} (R1, Open residual~\ref{res:y1}) and
  ...
  besides the cofiber R1a, the added $\Sym_{n-1}\to\Sym_n$ branching
  step R1b (Remark~\ref{rem:y1-branching}); and R3 is three
  sub-problems --- existence R3a, degree R3b, injectivity R3c --- of
  which existence and degree are upstream of injectivity. Both attacks
  require first pinning the cell-level spectral sequence of $\fobs$
  (Remark~\ref{rem:holim-prereq}).
...
\item \textbf{Do not Lean-formalize yet} (Phase~3). Phase~3 is gated on
  the independent re-validation of this revised document returning
  GREEN. Of the Fork~A content,
  Theorems~\ref{thm:projection},~\ref{thm:welldefined}, and~%
  \ref{thm:maschke} and Proposition~\ref{prop:y2a-mech} are the
  formalization-ready core; the residuals R1 (two-part), R2, and R3
  (three-part) are not.
\item \textbf{The consolidated paper (\texttt{../main.tex}) is not
  edited by Phase~1.} It honestly records the pre-Fork-A state. A
  later pass --- once the independent re-validation of this revised
  document returns GREEN --- may fold the Fork~A re-foundation into
  the paper, superseding its \S\S3--4. That is out of Phase-1 scope.
\end{enumerate}
...
(its target object R1a unchanged, but its reduction now carrying a new
...
structure of $(\Z/2)^n$ that $\Sym_n$ lacks), the $Y_2$ landing R2
...
named residuals R1 (two-part), R2, and R3 (three-part) --- and, in
the discipline the ledger's over-claim history demands, it withdraws
over-claims rather than carrying them forward: the
clean-landing-theorem over-claim caught in Phase~1, and the two
caught by the \texttt{mg-d300} Phase-2 validation and corrected in the
\texttt{mg-365e} revision (the ``$\Sym_n$-equivariant cofiber LES /
$R1$ unchanged'' over-claim and the ``exactly three residuals''
over-claim; Status~\ref{status:mg365e-revision}).
...
\item \textbf{Brief:} \texttt{mg-c15b} (Frankl-ForkA-Refound-LaTeX,
  Phase~1); \texttt{mg-365e} (Frankl-ForkA-Phase2-Revision --- the
  revision carried out in this document).
\item \textbf{The Phase-2 independent validation (the two findings
  ...
  revision spec); the revision is logged in
...
\texttt{mg-d300} Phase-2 validation. Deliverable on \texttt{main}:
this document (\texttt{paper/forkA/refoundation.tex}) and the ledger
notes \texttt{docs/Frankl-ForkA-Refound-LaTeX.md} (Phase~1) and
\texttt{docs/Frankl-ForkA-Phase2-Revision.md} (this revision).
...
$\Sym_{n-1}\to\Sym_n$ branching step), R2 ($Y_2$ landing, re-opened
and localized), and the three-part R3 (edge-map existence, degree, and
injectivity). Owed debt~1 (re-grading) is addressed; Owed debt~2
(faithfulness) is localized. Frankl is not proved; Phase~1 was not
meant to prove it. \emph{Revision note (\texttt{mg-365e}).} This
document has been revised following the \texttt{mg-d300} independent
Phase-2 validation, which returned AMBER and caught two surviving
over-claims: that the $Y_1$ cofiber-LES reduction is
...
three-part edge-map debt). Both are honest re-statements, not
refutations; the Fork~A strategy and its core constructions are
unchanged and were independently confirmed sound. An over-claim in an
earlier Phase-1 draft --- a clean ``standard-isotypic'' landing
theorem --- had already been caught and withdrawn. Recommended next:
independent re-validation of this revised document, then Phase~3
(Lean).
