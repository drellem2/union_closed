...
stuck-family fibres. The $Y_1$-closure Phase~1 did not compute it, but
sharpened it (Proposition~\ref{prop:y1-S-independent},
Remark~\ref{rem:y1-coefficient-blindness}): the residual diagram is the
\emph{same} for every $S$, so the computation is the
representation-theoretic question of whether the lower-Walsh coefficient
systems are blind to one fixed diagram --- and it isolated a foundational
prerequisite (Remark~\ref{rem:y1-prerequisite}), the cell-level
Bousfield--Kan structure maps, that stands upstream of the computation.
If this cohomology vanishes, $Y_1$ holds,
...
\item \textbf{Attack the $Y_1$ residual directly}
  (Open residual~\ref{res:y1}). First pin the cell-level Bousfield--Kan
  structure maps (Remark~\ref{rem:y1-prerequisite}) --- a bounded
  prerequisite standing upstream of any honest computation. Then, by the
  Phase-1 sharpening (Remark~\ref{rem:y1-coefficient-blindness}), the
  object to study is the family of coefficient systems $\{G_S\}$ on the
  fixed residual diagram, not the bare nerves of the comma categories
  $\rho_x/(T,\G)$: those nerves are $S$-independent and so cannot alone
  yield the $|S|$-stratified answer $Y_1$ needs. The smallest genuinely
  open instance is $n=3$, $S=\{1\}$, $k=1$, which should be computed by
  hand first (once the foundations are pinned). Do \emph{not} attempt a
  cofinality argument for $\rho_x$: by Correction~\ref{corr:r2} that is
  equivalent to the false $R2$.
