...
public import UnionClosed.Mathlib.Algebra.Homology.UCHomologicalBicomplex
public import Mathlib.Algebra.Module.Basic

/-!
# The total complex of a UC bicomplex (UnionClosed cascade-fork copy).

This file is a textual fork of `Mathlib/Algebra/Homology/TotalComplex.lean`
(mathlib `v4.29.1`, commit `5e932f97`), namespace-renamed to
`UCHomologicalComplex` / `UCHomologicalComplex₂`. This is sub-ticket 2
of the cascade-fork plan (mg-d079, depends on mg-f52c).

Given a preadditive category `C`, two complex shapes `c₁ : ComplexShape I₁`,
`c₂ : ComplexShape I₂`, a UC bicomplex `K : UCHomologicalComplex₂ C c₁ c₂`,
and a third complex shape `c₁₂ : ComplexShape I₁₂` equipped
with `[TotalComplexShape c₁ c₂ c₁₂]`, we construct the total complex
`K.total c₁₂ : UCHomologicalComplex C c₁₂`.

In particular, if `c := ComplexShape.up ℤ` and `K : UCHomologicalComplex₂ c c`, then for any
...
variable {C : Type*} [Category* C] [Preadditive C]
  ...
  (K L M : UCHomologicalComplex₂ C c₁ c₂) (φ : K ⟶ L) (e : K ≅ L) (ψ : L ⟶ M)
  (c₁₂ : ComplexShape I₁₂) [TotalComplexShape c₁ c₂ c₁₂]

/-- A UC bicomplex has a total bicomplex if for any `i₁₂ : I₁₂`, the coproduct
...
lemma hasTotal_of_iso [K.HasTotal c₁₂] : L.HasTotal c₁₂ :=
  GradedObject.hasMap_of_iso (GradedObject.isoMk K.toGradedObject L.toGradedObject
    (fun ⟨i₁, i₂⟩ =>
      (UCHomologicalComplex.eval _ _ i₁ ⋙ UCHomologicalComplex.eval _ _ i₂).mapIso e)) _
...
lemma d₂_eq_zero (h : ¬ c₂.Rel i₂ (c₂.next i₂)) :
    K.d₂ c₁₂ i₁ i₂ i₁₂ = 0 := by
  dsimp [d₂]
  rw [UCHomologicalComplex.shape _ _ _ h, zero_comp, smul_zero]

end

namespace totalAux
/-! Lemmas in the `totalAux` namespace should be used only in the internals of
the construction of the total complex `UCHomologicalComplex₂.total`. -/
    ...
    · ext ⟨i₁, i₂⟩ h
      ...
      · rw [totalAux.d₂_eq K c₁₂ i₁ h₃ i₁₂']; swap
        ...
        · rw [totalAux.d₂_eq K c₁₂ i₁ h₄ i₁₂'', Linear.comp_units_smul,
            UCHomologicalComplex.d_comp_d_assoc, zero_comp, smul_zero, smul_zero]
      ...
      · rw [totalAux.d₁_eq K c₁₂ h₃ i₂ i₁₂']; swap
        ...
        · have h₅ : ComplexShape.π c₁ c₂ c₁₂ (i₁, c₂.next i₂) = i₁₂' := by
          ...
          simp only [totalAux.d₂_eq K c₁₂ _ h₄ _ h₅, totalAux.d₂_eq K c₁₂ _ h₄ _ h₆,
            ...
            totalAux.d₁_eq K c₁₂ h₃ _ _ h₆, UCHomologicalComplex.Hom.comm_assoc, smul_smul,
...
/-- The total complex of a UC bicomplex. -/
@[simps -isSimp d]
noncomputable def total : UCHomologicalComplex C c₁₂ where
...
/-- Given a UC bicomplex `K`, this is a constructor for morphisms from `(K.total c₁₂).X i₁₂`. -/
...
/-- The morphism `K.total c₁₂ ⟶ L.total c₁₂` of UC homological complexes induced
by a morphism of UC bicomplexes `K ⟶ L`. -/
...
@[simp]
lemma forget_map :
    (UCHomologicalComplex.forget C c₁₂).map (map φ c₁₂) =
      GradedObject.mapMap (toGradedObjectMap φ) _ := rfl

variable (K) in
@[simp]
lemma map_id : map (𝟙 K) c₁₂ = 𝟙 _ := by
  apply (UCHomologicalComplex.forget _ _).map_injective
...
/-- The isomorphism `K.total c₁₂ ≅ L.total c₁₂` of UC homological complexes induced
by an isomorphism of UC bicomplexes `K ≅ L`. -/
@[simps]
...
variable (C c₁ c₂)
variable [∀ (K : UCHomologicalComplex₂ C c₁ c₂), K.HasTotal c₁₂]

/-- The functor which sends a UC bicomplex to its total complex. -/
@[simps]
noncomputable def totalFunctor :
    UCHomologicalComplex₂ C c₁ c₂ ⥤ UCHomologicalComplex C c₁₂ where
  obj K := K.total c₁₂
  map φ := total.map φ c₁₂
