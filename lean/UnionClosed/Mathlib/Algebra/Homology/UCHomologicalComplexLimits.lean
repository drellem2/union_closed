...
/-!
# Limits and colimits in the category of homological complexes (UnionClosed cascade-fork copy).

This file is a textual fork of
`Mathlib/Algebra/Homology/HomologicalComplexLimits.lean` (mathlib `v4.29.1`,
commit `5e932f97`), namespace-renamed to `UCHomologicalComplex`. The
`single`-dependent material from mathlib's `HomologicalComplexLimits.lean`
(originally lines 207-229: `PreservesLimitsOfShape J (single C c i)` and
companions) is **not forked** here, because the cascade fork does not
need a `UCHomologicalComplex.single` functor for the Sub-ticket-1
acceptance bars. See `docs/Frankl-cascade-fork-execution-plan.md` §2.

Joël Riou attribution preserved.
...
then it is also the case of the categories `UCHomologicalComplex C c`,
and the evaluation functors `eval C c i : UCHomologicalComplex C c ⥤ C`
commute to these.
...
variable (F : J ⥤ UCHomologicalComplex C c)

set_option backward.isDefEq.respectTransparency false in
/-- A cone in `UCHomologicalComplex C c` is limit if the induced cones obtained
by applying `eval C c i : UCHomologicalComplex C c ⥤ C` for all `i` are limit. -/
...
set_option backward.isDefEq.respectTransparency false in
/-- A cone for a functor `F : J ⥤ UCHomologicalComplex C c` which is given in degree `n` by
...
instance [HasLimitsOfShape J C] : HasLimitsOfShape J (UCHomologicalComplex C c) := ⟨inferInstance⟩

noncomputable instance [HasLimitsOfShape J C] (n : ι) :
    PreservesLimitsOfShape J (eval C c n) := ⟨inferInstance⟩

instance [HasFiniteLimits C] : HasFiniteLimits (UCHomologicalComplex C c) :=
  ⟨fun _ _ => inferInstance⟩

noncomputable instance [HasFiniteLimits C] (n : ι) : PreservesFiniteLimits (eval C c n) :=
  ⟨fun _ _ _ => inferInstance⟩

instance [HasFiniteLimits C] {K L : UCHomologicalComplex C c} (φ : K ⟶ L) [Mono φ] (n : ι) :
    Mono (φ.f n) := by
  change Mono ((UCHomologicalComplex.eval C c n).map φ)
  infer_instance
...
set_option backward.isDefEq.respectTransparency false in
/-- A cocone in `UCHomologicalComplex C c` is colimit if the induced cocones obtained
by applying `eval C c i : UCHomologicalComplex C c ⥤ C` for all `i` are colimit. -/
...
variable [∀ (n : ι), HasColimit (F ⋙ UCHomologicalComplex.eval C c n)]

set_option backward.isDefEq.respectTransparency false in
/-- A cocone for a functor `F : J ⥤ UCHomologicalComplex C c` which is given in degree `n` by
...
instance [HasColimitsOfShape J C] : HasColimitsOfShape J (UCHomologicalComplex C c) := ⟨inferInstance⟩

noncomputable instance [HasColimitsOfShape J C] (n : ι) :
    PreservesColimitsOfShape J (eval C c n) := ⟨inferInstance⟩

instance [HasFiniteColimits C] : HasFiniteColimits (UCHomologicalComplex C c) :=
  ⟨fun _ _ => inferInstance⟩

noncomputable instance [HasFiniteColimits C] (n : ι) :
    PreservesFiniteColimits (eval C c n) := ⟨fun _ _ _ => inferInstance⟩

instance [HasFiniteColimits C] {K L : UCHomologicalComplex C c} (φ : K ⟶ L) [Epi φ] (n : ι) :
    Epi (φ.f n) := by
  change Epi ((UCHomologicalComplex.eval C c n).map φ)
  infer_instance

/-- A functor `D ⥤ UCHomologicalComplex C c` preserves limits of shape `J`
if for any `i`, `G ⋙ eval C c i` does. -/
lemma preservesLimitsOfShape_of_eval {D : Type*} [Category* D]
    (G : D ⥤ UCHomologicalComplex C c)
...
/-- A functor `D ⥤ UCHomologicalComplex C c` preserves colimits of shape `J`
if for any `i`, `G ⋙ eval C c i` does. -/
