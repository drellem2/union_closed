...
/-!
# Homological complexes (UnionClosed cascade-fork copy).

This file is a textual fork of `Mathlib/Algebra/Homology/HomologicalComplex.lean`
(mathlib `v4.29.1`, commit `5e932f97`), with two local modifications:

* The bare instance `HasZeroMorphisms (HomologicalComplex V c)` at the
  original mathlib line 285 has been **deleted**. The fork's
  `UCHomologicalComplex V c` obtains `HasZeroMorphisms` exclusively
  through the preadditive path landed in
  `UnionClosed.Mathlib.Algebra.Homology.UCAdditive` (the
  `Preadditive (UCHomologicalComplex V c)` instance) composed with
  `CategoryTheory.Preadditive.preadditiveHasZeroMorphisms`. This eliminates
  the TC diamond between mathlib's `HomologicalComplex.instHasZeroMorphisms`
  and `preadditiveHasZeroMorphisms` that blocked `Mono`/`Epi` reasoning at
  the bicomplex ambient — see `docs/Frankl-cascade-fork-execution-plan.md`
  §2 for rationale and mg-7e53 for the original RED diagnosis.

* The underlying `Zero (X ⟶ Y)` instance at the original line 278 is
  **preserved**; it provides the zero morphism that the preadditive path
  consumes.

* All references `HomologicalComplex` → `UCHomologicalComplex`,
  `ChainComplex` → `UCChainComplex`, `CochainComplex` → `UCCochainComplex`
  (textual bulk rename; namespace shifted accordingly).

Local-only fork per Daniel directive 2026-05-17T13:53Z. Not for upstream PR.

The original mathlib documentation follows.

A `UCHomologicalComplex V c` with a "shape" controlled by `c : ComplexShape ι`
...
We provide `UCChainComplex V α` for
`α`-indexed chain complexes in which `d i j ≠ 0` only if `j + 1 = i`,
and similarly `UCCochainComplex V α`, with `i = j + 1`.

There is a category structure, where morphisms are chain maps.

For `C : UCHomologicalComplex V c`, we define `C.xNext i`, which is either `C.X j` for some
...
The composite of any two differentials `d i j ≫ d j k` must be zero.
-/
structure UCHomologicalComplex (c : ComplexShape ι) where
  X : ι → V
  d : ∀ i j, X i ⟶ X j
...
@[reassoc (attr := simp)]
theorem d_comp_d (C : UCHomologicalComplex V c) (i j k : ι) : C.d i j ≫ C.d j k = 0 := by
...
theorem ext {C₁ C₂ : UCHomologicalComplex V c} (h_X : C₁.X = C₂.X)
    (h_d :
      ∀ i j : ι,
...
def XIsoOfEq (K : UCHomologicalComplex V c) {p q : ι} (h : p = q) : K.X p ≅ K.X q :=
  eqToIso (by rw [h])

@[simp]
lemma XIsoOfEq_rfl (K : UCHomologicalComplex V c) (p : ι) :
    K.XIsoOfEq (rfl : p = p) = Iso.refl _ := rfl

@[reassoc (attr := simp)]
lemma XIsoOfEq_hom_comp_XIsoOfEq_hom (K : UCHomologicalComplex V c) {p₁ p₂ p₃ : ι}
    (h₁₂ : p₁ = p₂) (h₂₃ : p₂ = p₃) :
...
@[reassoc (attr := simp)]
lemma XIsoOfEq_hom_comp_XIsoOfEq_inv (K : UCHomologicalComplex V c) {p₁ p₂ p₃ : ι}
    (h₁₂ : p₁ = p₂) (h₃₂ : p₃ = p₂) :
...
@[reassoc (attr := simp)]
lemma XIsoOfEq_inv_comp_XIsoOfEq_hom (K : UCHomologicalComplex V c) {p₁ p₂ p₃ : ι}
    (h₂₁ : p₂ = p₁) (h₂₃ : p₂ = p₃) :
...
@[reassoc (attr := simp)]
lemma XIsoOfEq_inv_comp_XIsoOfEq_inv (K : UCHomologicalComplex V c) {p₁ p₂ p₃ : ι}
    (h₂₁ : p₂ = p₁) (h₃₂ : p₃ = p₂) :
...
@[reassoc (attr := simp)]
lemma XIsoOfEq_hom_comp_d (K : UCHomologicalComplex V c) {p₁ p₂ : ι} (h : p₁ = p₂) (p₃ : ι) :
    (K.XIsoOfEq h).hom ≫ K.d p₂ p₃ = K.d p₁ p₃ := by subst h; simp

@[reassoc (attr := simp)]
lemma XIsoOfEq_inv_comp_d (K : UCHomologicalComplex V c) {p₂ p₁ : ι} (h : p₂ = p₁) (p₃ : ι) :
    (K.XIsoOfEq h).inv ≫ K.d p₂ p₃ = K.d p₁ p₃ := by subst h; simp

@[reassoc (attr := simp)]
lemma d_comp_XIsoOfEq_hom (K : UCHomologicalComplex V c) {p₂ p₃ : ι} (h : p₂ = p₃) (p₁ : ι) :
    K.d p₁ p₂ ≫ (K.XIsoOfEq h).hom = K.d p₁ p₃ := by subst h; simp

@[reassoc (attr := simp)]
lemma d_comp_XIsoOfEq_inv (K : UCHomologicalComplex V c) {p₂ p₃ : ι} (h : p₃ = p₂) (p₁ : ι) :
    K.d p₁ p₂ ≫ (K.XIsoOfEq h).inv = K.d p₁ p₃ := by subst h; simp

end UCHomologicalComplex

/-- An `α`-indexed chain complex is a `UCHomologicalComplex`
in which `d i j ≠ 0` only if `j + 1 = i`.
-/
abbrev UCChainComplex (α : Type*) [AddRightCancelSemigroup α] [One α] : Type _ :=
  UCHomologicalComplex V (ComplexShape.down α)

/-- An `α`-indexed cochain complex is a `UCHomologicalComplex`
in which `d i j ≠ 0` only if `i + 1 = j`.
-/
abbrev UCCochainComplex (α : Type*) [AddRightCancelSemigroup α] [One α] : Type _ :=
  UCHomologicalComplex V (ComplexShape.up α)
...
end UCCochainComplex

namespace UCHomologicalComplex

variable {V}
variable {c : ComplexShape ι} (C : UCHomologicalComplex V c)

/-- A morphism of homological complexes consists of maps between the chain groups,
commuting with the differentials.
-/
@[ext]
structure Hom (A B : UCHomologicalComplex V c) where
  f : ∀ i, A.X i ⟶ B.X i
  comm' : ∀ i j, c.Rel i j → f i ≫ B.d i j = A.d i j ≫ f j := by cat_disch

@[reassoc (attr := simp)]
theorem Hom.comm {A B : UCHomologicalComplex V c} (f : A.Hom B) (i j : ι) :
    f.f i ≫ B.d i j = A.d i j ≫ f.f j := by
  by_cases hij : c.Rel i j
  · exact f.comm' i j hij
  · rw [A.shape i j hij, B.shape i j hij, comp_zero, zero_comp]

instance (A B : UCHomologicalComplex V c) : Inhabited (Hom A B) :=
  ⟨{ f := fun _ => 0 }⟩

/-- Identity chain map. -/
def id (A : UCHomologicalComplex V c) : Hom A A where f _ := 𝟙 _

/-- Composition of chain maps. -/
def comp (A B C : UCHomologicalComplex V c) (φ : Hom A B) (ψ : Hom B C) : Hom A C where
  f i := φ.f i ≫ ψ.f i
...
instance : Category (UCHomologicalComplex V c) where
  Hom := Hom
  id := id
  comp := comp _ _ _
...
@[ext]
lemma hom_ext {C D : UCHomologicalComplex V c} (f g : C ⟶ D)
...
@[simp]
theorem id_f (C : UCHomologicalComplex V c) (i : ι) : Hom.f (𝟙 C) i = 𝟙 (C.X i) :=
  rfl

@[simp, reassoc]
theorem comp_f {C₁ C₂ C₃ : UCHomologicalComplex V c} (f : C₁ ⟶ C₂) (g : C₂ ⟶ C₃) (i : ι) :
    (f ≫ g).f i = f.f i ≫ g.f i :=
  rfl

@[simp]
theorem eqToHom_f {C₁ C₂ : UCHomologicalComplex V c} (h : C₁ = C₂) (n : ι) :
    UCHomologicalComplex.Hom.f (eqToHom h) n =
      eqToHom (congr_fun (congr_arg UCHomologicalComplex.X h) n) := by
  subst h
  rfl

-- We'll use this later to show that `UCHomologicalComplex V c` is preadditive when `V` is.
theorem hom_f_injective {C₁ C₂ : UCHomologicalComplex V c} :
    Function.Injective fun f : Hom C₁ C₂ => f.f := by cat_disch

instance (X Y : UCHomologicalComplex V c) : Zero (X ⟶ Y) :=
  ⟨{ f := fun _ => 0}⟩

@[simp]
theorem zero_f (C D : UCHomologicalComplex V c) (i : ι) : (0 : C ⟶ D).f i = 0 :=
  rfl
...
/-- The zero complex -/
noncomputable def zero [HasZeroObject V] : UCHomologicalComplex V c where
  X _ := 0
  d _ _ := 0

theorem isZero_zero [HasZeroObject V] : IsZero (zero : UCHomologicalComplex V c) := by
...
instance [HasZeroObject V] : HasZeroObject (UCHomologicalComplex V c) :=
  ⟨⟨zero, isZero_zero⟩⟩

noncomputable instance [HasZeroObject V] : Inhabited (UCHomologicalComplex V c) :=
  ⟨zero⟩

theorem congr_hom {C D : UCHomologicalComplex V c} {f g : C ⟶ D} (w : f = g) (i : ι) :
    f.f i = g.f i :=
  congr_fun (congr_arg Hom.f w) i

lemma mono_of_mono_f {K L : UCHomologicalComplex V c} (φ : K ⟶ L)
...
lemma epi_of_epi_f {K L : UCHomologicalComplex V c} (φ : K ⟶ L)
...
@[simps]
def eval (i : ι) : UCHomologicalComplex V c ⥤ V where
  obj C := C.X i
  map f := f.f i

-- The instance `(eval V c i).PreservesZeroMorphisms` from mathlib's
-- original `HomologicalComplex.lean` has been moved to `UCAdditive.lean`
-- (cascade-fork sub-ticket 1, mg-f52c): it requires
-- `HasZeroMorphisms (UCHomologicalComplex V c)`, which after deletion of
-- the direct mathlib instance is provided only via the preadditive path
-- under `[Preadditive V]`. The instance is therefore declared in the
-- file that establishes `Preadditive (UCHomologicalComplex V c)`.

/-- The functor forgetting the differential in a complex, obtaining a graded object. -/
@[simps]
def forget : UCHomologicalComplex V c ⥤ GradedObject ι V where
  obj C := C.X
  map f := f.f
...
@[reassoc]
lemma XIsoOfEq_hom_naturality {K L : UCHomologicalComplex V c} (φ : K ⟶ L) {n n' : ι} (h : n = n') :
    φ.f n ≫ (L.XIsoOfEq h).hom = (K.XIsoOfEq h).hom ≫ φ.f n' := by subst h; simp

@[reassoc]
lemma XIsoOfEq_inv_naturality {K L : UCHomologicalComplex V c} (φ : K ⟶ L) {n n' : ι} (h : n = n') :
...
variable {C₁ C₂ C₃ : UCHomologicalComplex V c}
...
theorem isIso_of_components (f : C₁ ⟶ C₂) [∀ n : ι, IsIso (f.f n)] : IsIso f :=
  (UCHomologicalComplex.Hom.isoOfComponents fun n => asIso (f.f n)).isIso_hom
...
@[simp]
theorem sqFrom_id (C₁ : UCHomologicalComplex V c) (i : ι) : sqFrom (𝟙 C₁) i = 𝟙 _ :=
  rfl
...
-/
def of (X : α → V) (d : ∀ n, X (n + 1) ⟶ X n) (sq : ∀ n, d (n + 1) ≫ d n = 0) : UCChainComplex V α :=
  { X := X
...
set_option backward.isDefEq.respectTransparency false in
/-- A constructor for chain maps between `α`-indexed chain complexes built using `UCChainComplex.of`,
...
variable {V}
variable (P Q : UCChainComplex V ℕ) (zero : P.X 0 ⟶ Q.X 0) (one : P.X 1 ⟶ Q.X 1)
...
set_option backward.isDefEq.respectTransparency false in
/--
A constructor for chain maps between `α`-indexed cochain complexes built using `UCCochainComplex.of`,
from a dependently typed collection of morphisms.
-/
@[simps]
...
variable {V}
variable (P Q : UCCochainComplex V ℕ) (zero : P.X 0 ⟶ Q.X 0) (one : P.X 1 ⟶ Q.X 1)
